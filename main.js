...
(function () {
  ...
  document.querySelectorAll('.poi-item[data-dest]').forEach((item) => {
    item.addEventListener('click', () => {
      ...
      if (wasActive) {
        ...
        mapFrame.src = 'https://www.google.com/maps?saddr=' + encodeURIComponent('Caroline Øverlands vei 49, 1356 Bekkestua') + '&daddr=' + encodeURIComponent(dest) + '&dirflg=w&output=embed';
      }
    });
  });
  ...
  window.handleSubmit = async (event) => {
    ...
    try {
      ...
      alert('Det oppsto en feil. Vennligst prøv igjen senere eller ring megler direkte.');
